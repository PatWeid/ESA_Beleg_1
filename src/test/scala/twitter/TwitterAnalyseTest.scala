...
class TwitterAnalyseTest extends AnyFunSuite with BeforeAndAfterAll {
  ...
  val tweets= twitterDataFromFile("tweets2021.json", true)
  ...
  test("test and conjunctionComplicated 1") {
    val expected = HashSet()
    val result = TwitterTextProblems.andConjunctionComplicated(List("vollpfosten", "idiot"), invInd)
    assert(result === expected)
  }

  test("test and conjunctionComplicated 2") {
    val expected = Set(1346091425478430720L)
    val result = TwitterTextProblems.andConjunctionComplicated(List("menschenverachtender", "idiot"), invInd)
    assert(result === expected)
  }

  test("test and conjunctionOli 1") {
    val expected = HashSet()
    val result = TwitterTextProblems.andConjunctionOli(List("vollpfosten", "idiot"), invInd)
    assert(result === expected)
  }

  test("test and conjunctionOli 2") {
    val expected = Set(1346091425478430720L)
    val result = TwitterTextProblems.andConjunctionOli(List("menschenverachtender", "idiot"), invInd)
    assert(result === expected)
  }
